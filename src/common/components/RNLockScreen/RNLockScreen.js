...
class RNLockScreen extends Component {
  ...
  _onCapture = lock => {
    const {onCapture, type} = this.props;
    const {username} = this.state;

    if (type === RNLockScreen.Type.Pin) {
      onCapture && onCapture({lock, username});
    }
  ...
  _renderLockFragment() {
    ...
    if (renderLockFragment) {
      return renderLockFragment();
    }
    ...
    if (type === RNLockScreen.Type.Pin) {
      ...
      return (
        <PatternFragment
          ...
          onDone={this._onDone}
          clear={
            this.props.state === HeaderFragment.State.Default ? false : true
          }
          lock={lock}
          style={containerProps}
          {...lockFragmentProps}
      ...
      return (
        <ImageBackground
          ...
          <Toast
            ...
            textStyle={style.toastTextStyle}
          />
        </ImageBackground>
      );
    } else {
...
const State = HeaderFragment.State;
export {RNLockScreen, State};
